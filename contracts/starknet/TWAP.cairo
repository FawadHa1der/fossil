...
@event
func computation_finalized(computation_id: felt) {
}
...
@view
func get_twap_computation_cache{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    computation_id: felt, slot: felt
) -> (res: felt) {
...
@external
func register_computation{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    start_block: felt, end_block: felt, avg_header_param: felt, callback_address: felt
) {
    alloc_locals;
    ...
    let (local current_block_hash) = IL1HeadersStore.get_parent_hash(
        l1_headers_store_addr, start_block + 1
    );
...
func compute{
    ...
    let (
        ...
        local iterations: felt,
    ) = compute_rec{keccak_ptr=keccak_ptr}(
        avg_header_param,
        current_block,
        current_block_hash,
    ...
    local is_population_full = is_le(last_block_number, end_block);
...
func retrieve_param{range_check_ptr}(header_rlp: IntsSequence, avg_header_param: felt) -> (
    res: felt
) {
