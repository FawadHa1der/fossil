...
func count_shared_prefix_len{range_check_ptr}(
    path_offset: felt, path: IntsSequence, element_rlp: IntsSequence, node_path_item: RLPItem
) -> (new_path_offset: felt) {
    alloc_locals;

    let (local node_path_decoded: IntsSequence) = extract_data(
        node_path_item.dataPosition, node_path_item.length, element_rlp
    );
...
func count_shared_prefix_len_rec{range_check_ptr}(
    ...
    let (current_node_path_nibble) = extract_nibble_from_words(
        node_path_decoded, current_index + skip_nibbles
    );

    if (current_path_nibble == current_node_path_nibble) {
        return count_shared_prefix_len_rec(
            path_offset, path, node_path_decoded, skip_nibbles, current_index + 1
        );
    } else {
        return (current_index,);
    }
...
func get_next_hash{range_check_ptr}(rlp_input: IntsSequence, rlp_node: RLPItem) -> (
    res: IntsSequence
) {
    alloc_locals;
...
func verify_proof{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    path: IntsSequence, root_hash: IntsSequence, proof: IntsSequence*, proof_len: felt
) -> (res: IntsSequence) {
    alloc_locals;
...
func verify_proof_rec{keccak_ptr: felt*, range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    ...
    if (current_index == 0) {
        let (local hashes_match: felt) = arr_eq(
            current_element_keccak.element,
            current_element_keccak.element_size_words,
            root_hash.element,
            root_hash.element_size_words,
        );
    ...
    if (node_len == 2) {
        ...
        if (current_index == proof_len - 1) {
            ...
            let (local res: IntsSequence) = extract_data(
                node[1].dataPosition, node[1].length, current_element
            );
            return (res,);
        } else {
            ...
            if (is_list == 0) {
                ...
                let (local element_data_extracted: IntsSequence) = extract_data(
                    children.dataPosition, children.length, current_element
                );
                let (local next_hash_words_le: felt*) = keccak256{keccak_ptr=keccak_ptr}(
                    element_data_extracted
                );
        ...
        if (current_index == proof_len - 1) {
            if (path_offset + 1 == path.element_size_bytes * 2) {
                let (local res: IntsSequence) = extract_data(
                    node[16].dataPosition, node[16].length, current_element
                );
                return (res,);
            } else {
